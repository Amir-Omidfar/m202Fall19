...
// todo: 
//register path list to get constant node-to-node distance 
//find a new data-structure to store walking path info instead of singly linked list (decision-making graph)
//in corporate IMU data to store different height 
    ...
    for (anchor of MINUET.anchors) {
      ...
      addABall(posAnchor, 0x0000ff, 150);
          ...
          function MQTTconnect() {
            ...
            mqtt = new Paho.MQTT.Client("192.168.1.40",9001,"clientjs");
            var options = {
              timeout: 3,
              onSuccess: onConnect,
              onFailure: onFailure,
...
MINUET.updateCurPoint = function(rawData){
    ...
    //console.log("data_stream "+ data_stream[0]);
    //console.log("data_stream "+ data_stream[1]);
    //console.log("data_stream "+ data_stream[2]);
    //console.log("data_stream "+ data_stream[3]);
    ...
    var x = -(Number(data_stream[0][0])+Number(data_stream[1][0])+Number(data_stream[2][0])+Number(data_stream[3][0]))/4;
    var y = (Number(data_stream[0][1])+Number(data_stream[1][1])+Number(data_stream[2][1])+Number(data_stream[3][1]))/4;
    //var proxyZ = Number(singleData[2]);
    //var proxyZ = Number(singleData[2])-Math.sin((pitch * Math.PI) / 180)*MINUET.armLength;
    var z = MINUET.hardcodedZ;
    //var z = 0;
...
document.addEventListener("keypress", function(event) {
  ...
  if(event.keyCode == 49){
    ...
    if(Object.keys(fur_dict).length != 0){
       ...
       var i_start = findNearestNode(path_list,MINUET.user.position); 
       var i_end = findNearestNode(path_list,destination); 
